...
routes = [
    ...
    Route(
        ...
        method="GET",
        handler=str_query_param_with_default,
    ),
    Route(
        url="/int_query_param_with_default/",
        method="GET",
        handler=int_query_param_with_default,
    ),
    Route(
        url="/bool_query_param_with_default/",
        method="GET",
        handler=bool_query_param_with_default,
    ),
    # Body parameters
...
def test_type_body_param2():
    response = client.post(
        "/type_body_param2/", json={"api_key": "x" * 32, "api_secret": "y" * 16}
    )
    assert response.json() == {"api_key": {"api_key": "x" * 32, "api_secret": "y" * 16}}

    response = client.post(
        "/type_body_param2/", json={"api_key": "x", "api_secret": "y" * 16}
    )
    assert response.status_code == 400
    assert response.json() == {"api_key": "Must have exactly 32 characters."}

    response = client.post(
        "/type_body_param2/", json={"api_key": "x" * 32, "api_secret": "y"}
    )
    assert response.status_code == 400
    assert response.json() == {"api_secret": "Must have exactly 16 characters."}

    response = client.post(
        "/type_body_param2/", json={"api_key": "x" * 100, "api_secret": "y" * 16}
    )
    assert response.status_code == 400
    assert response.json() == {"api_key": "Must have exactly 32 characters."}

    response = client.post(
        "/type_body_param2/", json={"api_key": "x" * 32, "api_secret": "y" * 100}
    )
    assert response.status_code == 400
    assert response.json() == {"api_secret": "Must have exactly 16 characters."}
