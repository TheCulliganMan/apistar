...
class App:
    ...
    def init_templates(
        self, template_dir: str = None, packages: typing.Sequence[str] = None
    ):
    ...
    def get_event_hooks(self):
        ...
        on_error = [
            hook.on_error for hook in reversed(event_hooks) if hasattr(hook, "on_error")
        ]
    ...
    def __call__(self, environ, start_response):
        ...
        try:
            ...
            try:
                state["exc"] = exc
                funcs = [self.exception_handler] + on_response + [self.finalize_wsgi]
                return self.injector.run(funcs, state)
        ...
        async def asgi_callable(receive, send):
            ...
            try:
                ...
                try:
                    state["exc"] = exc
                    funcs = (
                        [self.exception_handler] + on_response + [self.finalize_asgi]
                    )
                    await self.injector.run_async(funcs, state)
                except Exception as inner_exc:
                    try:
